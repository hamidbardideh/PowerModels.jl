function run_opst(file, model_type::Type, optimizer; kwargs...)
    data = PowerModels.parse_file(file)
    process_pst_data!(data)
    return run_model(data, model_type, optimizer, build_opst; ref_extensions = [ref_add_pst!], kwargs...)
end

""
function build_opst(pm::AbstractPowerModel)
    variable_voltage(pm)
    ...
    for i in ids(pm, :bus)
        constraint_power_balance_pst(pm, i)
    end
...
function variable_active_pst_flow(pm::AbstractPowerModel; nw::Int=pm.cnw, bounded::Bool=true, report::Bool=true)
    ...
    report && sol_component_value_edge(pm, nw, :pst, :pf, :pt, ref(pm, nw, :arcs_from_pst), ref(pm, nw, :arcs_to_pst), p)
end

"variable: `q[l,i,j]` for `(l,i,j)` in `arcs`"
function variable_reactive_pst_flow(pm::AbstractPowerModel; nw::Int=pm.cnw, bounded::Bool=true, report::Bool=true)
    ...
    report && sol_component_value_edge(pm, nw, :pst, :qf, :qt, ref(pm, nw, :arcs_from_pst), ref(pm, nw, :arcs_to_pst), q)
end
...
function variable_pst_angle(pm::AbstractPowerModel; nw::Int=pm.cnw, bounded::Bool=true, report::Bool=true)
    ...
    report && sol_component_value(pm, nw, :pst, :alpha, ids(pm, nw, :pst), alpha)
end

###############################################
# Constraint templates
function constraint_ohms_y_from_pst(pm::AbstractPowerModel, i::Int; nw::Int=pm.cnw)
    pst = ref(pm, nw, :pst, i)
    f_bus = pst["fbus"]
    t_bus = pst["tbus"]
    f_idx = (i, f_bus, t_bus)
...
function constraint_ohms_y_to_pst(pm::AbstractPowerModel, i::Int; nw::Int=pm.cnw)
    pst = ref(pm, nw, :pst, i)
    f_bus = pst["fbus"]
    t_bus = pst["tbus"]
    f_idx = (i, f_bus, t_bus)
...
function constraint_power_balance_pst(pm::AbstractPowerModel, i::Int; nw::Int=pm.cnw)
    bus = ref(pm, nw, :bus, i)
    ...
    constraint_power_balance_pst(pm, nw, i, bus_arcs, bus_arcs_pst, bus_arcs_dc, bus_arcs_sw, bus_gens, bus_storage, bus_pd, bus_qd, bus_gs, bus_bs)
end

###############################################
# PST Constraints
function constraint_ohms_y_from_pst(pm::AbstractACPModel, n::Int, i::Int, f_bus, t_bus, f_idx, t_idx, g, b, g_fr, b_fr)
...
function constraint_ohms_y_to_pst(pm::AbstractACPModel, n::Int, i::Int, f_bus, t_bus, f_idx, t_idx, g, b, g_to, b_to)
    alpha = var(pm, n,  :psta, i)
...
function constraint_limits_pst(pm::AbstractPowerModel, i::Int; nw::Int=pm.cnw)
    pst = ref(pm, nw, :pst, i)
    srated = pst["rateA"]
    angmin = pst["angmin"]
    angmax = pst["angmax"]
...
function constraint_power_balance_pst(pm::AbstractACPModel, n::Int, i::Int, bus_arcs, bus_arcs_pst, bus_arcs_dc, bus_arcs_sw, bus_gens, bus_storage, bus_pd, bus_qd, bus_gs, bus_bs)
    vm   = var(pm, n, :vm, i)
    ...
    ppst    = get(var(pm, n),    :ppst, Dict()); _check_var_keys(p, bus_arcs_pst, "active power", "pst")
    qpst    = get(var(pm, n),    :qpst, Dict()); _check_var_keys(q, bus_arcs_pst, "reactive power", "pst")
...
###############################################
# ADD REF MODEL
function ref_add_pst!(pm::AbstractPowerModel)
...
function scale_pst_data!(pst, MVAbase)
    ...
    _apply_func!(pst, "rateA", rescale_power)
    _apply_func!(pst, "rateB", rescale_power)
    _apply_func!(pst, "rateC", rescale_power)
    _apply_func!(pst, "angle", deg2rad)
    _apply_func!(pst, "angmin", deg2rad)
    _apply_func!(pst, "angmax", deg2rad)
end
