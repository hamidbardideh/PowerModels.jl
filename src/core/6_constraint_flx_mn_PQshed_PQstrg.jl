...
function constraint_storage_new_state(pm::AbstractPowerModel, i::Int; nw::Int=pm.cnw)
    ...
    constraint_storage_new_state_initial(pm, nw, i, storage["energy"], storage["charge_efficiency"], storage["discharge_efficiency"],   storage["ext_inj_energy"], storage["lost_wasted_energy"], time_elapsed)
end
...
#2. I think it should be se_1-se_2(make an issue)
    JuMP.@constraint(pm.model, se_2 - se_1 == time_elapsed*(charge_eff*sc_2 - sd_2/discharge_eff) + ext_inj_energy - lost_wasted_energy )
end
...
function constraint_storage_new_state(pm::AbstractPowerModel, i::Int, nw_1::Int, nw_2::Int)
    ...
    if haskey(ref(pm, nw_1, :ne_storage), i)
        ...
        # if the storage device has status=0 in nw_1, then the stored energy variable will not exist. Initialize storage from data model instead.
        Memento.warn(_LOGGER, "storage component $(i) was not found in network $(nw_1) while building constraint_storage_state between networks $(nw_1) and $(nw_2). Using the energy value from the storage component in network $(nw_2) instead")
        constraint_storage_new_state_initial(pm, nw_2, i, storage["energy"], storage["charge_efficiency"], storage["discharge_efficiency"],storage["ext_inj_energy"], storage["lost_wasted_energy"], time_elapsed)
    end
end
...
# " 2. charging/discharging exlusivity"
# function constraint_storage_new_chr_dischr_exclusivity(pm::AbstractPowerModel, i::Int; nw::Int=pm.cnw)
...
" 2. charging/discharging exlusivity"
function constraint_PQstrg_new_chr_dischr_exclusivity(pm::AbstractPowerModel, i::Int; nw::Int=pm.cnw)
    ...
    qs_dch_new = var(pm, nw, :qs_dch_new, i)
    ...
    JuMP.@constraint(pm.model,  ps_ch_new <= max_strg_ch * z_bin * Z_strg )
    JuMP.@constraint(pm.model,  (ps_dch_new ^ 2) + (qs_dch_new ^2) <= ((max_strg_disch * (1-z_bin)) ^2) )
    JuMP.@constraint(pm.model,  ps_dch_new <=0  )
    JuMP.@constraint(pm.model,  qs_dch_new <=0  )
...
function constraint_storage_new_energy_limit(pm::AbstractPowerModel, i::Int; nw::Int=pm.cnw)
    ...
    JuMP.@constraint(pm.model, min_strg_limit <= se_new <= max_strg_limit)
end
...
function constraint_storage_new_dischrg_limit(pm::AbstractPowerModel, i::Int; nw::Int=pm.cnw)
    ...
    JuMP.@constraint(pm.model, min_strg_disch <= sd_new <= max_strg_disch)
end
...
function constraint_power_balance_flx_ne_PQshed_PQstrg(pm::AbstractPowerModel, i::Int; nw::Int=pm.cnw)
    ...
    constraint_power_balance_flx_ne_PQshed_PQstrg(pm, nw, i, bus_arcs, bus_arcs_dc, bus_arcs_sw, bus_arcs_ne, bus_gens, bus_storage, bus_storage_new, bus_gs, bus_bs, bus_loads)
end

function constraint_power_balance_flx_ne_PQshed_PQstrg(pm::AbstractPowerModel, n::Int, i, bus_arcs, bus_arcs_dc, bus_arcs_sw, bus_arcs_ne, bus_gens, bus_storage, bus_storage_new, bus_gs, bus_bs, bus_loads)
# get(collection, key, default)           :Return the value stored for the given key, or the given default value if no mapping for the key is present.
        ...
        Z_strg = get(var(pm,n), :storage_ne, Dict())                            # this is a binary decision varaiable for new storages
        ...
        qs_dch_new = get(var(pm, n),  :qs_dch_new, Dict())
        ...
        ==
            ...
            + sum(qs_dch_new[sn]*Z_strg[sn] for sn in bus_storage_new)
...
"9.  constraint sc_sd relationship with ps_ch ps_dch qs_dch   "
function constraint_storage_new_sc_ps_and_sd_pd_qd(pm::AbstractPowerModel, i::Int; nw::Int=pm.cnw)
    ...
    JuMP.@constraint(pm.model, sd_new ^ 2 == ((ps_dch_new ^ 2) + (qs_dch_new ^ 2))  )
