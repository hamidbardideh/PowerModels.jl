...
function variable_ne_storage_PQ(pm::AbstractPowerModel; kwargs...) # it reflects in Prblem definition file!===>    DONE
    # variable_active_ne_storage(pm; kwargs...)
    variable_ne_storage_charge(pm; kwargs...)
    variable_ne_storage_discharge(pm; kwargs...)
    variable_z_binary(pm; kwargs...)
    variable_ne_storage_active_ch(pm; kwargs...)
    variable_ne_storage_active_dch(pm; kwargs...)
    variable_ne_storage_reactive_dch(pm; kwargs...)                             #additional
...
"variable: `0 <= branch_ne[l] <= 1` for `l` in `branch`es"
function variable_z_binary(pm::AbstractPowerModel; nw::Int=pm.cnw, relax::Bool=false, report::Bool=true)
    if !relax
    storage_ch_dch_bin = var(pm, nw)[:storage_ch_dch_bin] = JuMP.@variable(pm.model,
        [l in ids(pm, nw, :ne_storage)], base_name="$(nw)_storage_ch_dch_bin",
        binary = true,
         start = comp_start_value(ref(pm, nw, :ne_storage, l), "storage_ch_dch_bin_start", 1.0)
    )
    else
    storage_ch_dch_bin = var(pm, nw)[:storage_ch_dch_bin] = JuMP.@variable(pm.model,
        [l in ids(pm, nw, :ne_storage)], base_name="$(nw)_storage_ch_dch_bin",
...
function variable_ne_storage_active_ch(pm::AbstractPowerModel; nw::Int=pm.cnw, bounded::Bool=true, report::Bool=true)
    ps_ch_new = var(pm, nw)[:ps_ch_new] = JuMP.@variable(pm.model,
        [i in ids(pm, nw, :ne_storage)], base_name="$(nw)_ps__ch_new",
         start = comp_start_value(ref(pm, nw, :ne_storage, i), "ps_ch_new_start")
    )
    ...
    if bounded
        ...
        for i in ids(pm, nw, :ne_storage)
            if !isinf(inj_lb[i])
                JuMP.set_lower_bound(ps_ch_new[i], inj_lb[i])
            end
            if !isinf(inj_ub[i])
                JuMP.set_upper_bound(ps_ch_new[i], inj_ub[i])
            end
        end
    end

    report && _IM.sol_component_value(pm, nw, :ne_storage, :ps_ch_new, ids(pm, nw, :ne_storage), ps_ch_new)
end



function variable_ne_storage_active_dch(pm::AbstractPowerModel; nw::Int=pm.cnw, bounded::Bool=true, report::Bool=true)
    ps_dch_new = var(pm, nw)[:ps_dch_new] = JuMP.@variable(pm.model,
        [i in ids(pm, nw, :ne_storage)], base_name="$(nw)_ps_dch_new",
         start = comp_start_value(ref(pm, nw, :ne_storage, i), "ps_dch_new_start")
    )
    ...
    if bounded
        ...
        for i in ids(pm, nw, :ne_storage)
            if !isinf(inj_lb[i])
                JuMP.set_lower_bound(ps_dch_new[i], inj_lb[i])
            end
            if !isinf(inj_ub[i])
                JuMP.set_upper_bound(ps_dch_new[i], inj_ub[i])
            end
        end
    end

    report && _IM.sol_component_value(pm, nw, :ne_storage, :ps_dch_new, ids(pm, nw, :ne_storage), ps_dch_new)
end

""

# function variable_ne_storage_reactive_ch(pm::AbstractPowerModel; nw::Int=pm.cnw, bounded::Bool=true, report::Bool=true)               $ REACTIVE CHARGE DOES NOT EXIST
#     qs_new = var(pm, nw)[:qs_ch_new] = JuMP.@variable(pm.model,
#         [i in ids(pm, nw, :ne_storage)], base_name="$(nw)_qs_ch_new",
#          start = comp_start_value(ref(pm, nw, :ne_storage, i), "qs_ch_new_start")
#     )
#
#     if bounded
#         inj_lb, inj_ub = ref_calc_storage_injection_bounds(ref(pm, nw, :ne_storage), ref(pm, nw, :bus))
...
#             JuMP.set_lower_bound(qs_ch_new[i], max(inj_lb[i], storage["qmin"]))
#             JuMP.set_upper_bound(qs_ch_new[i], min(inj_ub[i], storage["qmax"]))
#         end
#     end
#
#     report && _IM.sol_component_value(pm, nw, :ne_storage, :qs_ch_new, ids(pm, nw, :ne_storage), qs_ch_new)
# end


function variable_ne_storage_reactive_dch(pm::AbstractPowerModel; nw::Int=pm.cnw, bounded::Bool=true, report::Bool=true)
    qs_dch_new = var(pm, nw)[:qs_dch_new] = JuMP.@variable(pm.model,
        [i in ids(pm, nw, :ne_storage)], base_name="$(nw)_qs_dch_new",
         start = comp_start_value(ref(pm, nw, :ne_storage, i), "qs_dch_new_start")
    )
    ...
    if bounded
        ...
        for (i, storage) in ref(pm, nw, :ne_storage)
            JuMP.set_lower_bound(qs_dch_new[i], max(inj_lb[i], storage["qmin"]))
            JuMP.set_upper_bound(qs_dch_new[i], min(inj_ub[i], storage["qmax"]))
        end
    end

    report && _IM.sol_component_value(pm, nw, :ne_storage, :qs_dch_new, ids(pm, nw, :ne_storage), qs_dch_new)
end
