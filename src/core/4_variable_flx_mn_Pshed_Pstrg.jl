...
function variable_storage_ne(pm::AbstractPowerModel; nw::Int=pm.cnw, relax::Bool=false, report::Bool=true)
    ...
    report && _IM.sol_component_value(pm, nw, :ne_storage, :to_bus, ids(pm, nw, :ne_storage), storage_ne)
end


# 2. all storage variables were redefined for new_storage
"variables for modeling storage units, includes grid injection and internal variables"
function variable_ne_storage_P(pm::AbstractPowerModel; kwargs...) # it reflects in Prblem definition file!===>    DONE
...
function variable_ne_storage_active_ch(pm::AbstractPowerModel; nw::Int=pm.cnw, bounded::Bool=true, report::Bool=true)
    ps_ch_new = var(pm, nw)[:ps_ch_new] = JuMP.@variable(pm.model,
        [i in ids(pm, nw, :ne_storage)], base_name="$(nw)_ps_ch_new",
...
function variable_ne_storage_current(pm::AbstractPowerModel; nw::Int=pm.cnw, bounded::Bool=true, report::Bool=true)
end
