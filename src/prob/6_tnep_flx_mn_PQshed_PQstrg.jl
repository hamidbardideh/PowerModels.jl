...
function run_opf_mn_flx(file, model_type::Type, optimizer; kwargs...)
    return run_model(file, model_type, optimizer, build_mn_opf_flx; ref_extensions= [ref_add_on_off_va_bounds!,ref_add_ne_branch!, ref_add_ne_storage!], kwargs...)
end



function build_mn_opf_flx(pm::AbstractPowerModel)
...
#variables in already existing storages
        variable_storage_mi(pm, nw=n)           # Includes all variables that need to be defined for storages

# other variables for new storage multinetwork problem
        variable_storage_ne(pm, nw=n)           # binary decision variable for storage_ne
        variable_ne_storage_PQ(pm, nw=n)           # Includes all variables need to be defined for variable_storage_ne
...
# Power Flow constraints
        ...
        for i in ids(pm, :bus, nw=n)            # Nodal balance equation (the equations is modifed with respect to tnep_gen_sh.jl , in addition to the flexible load,   1.new storage modify the power balance equation 2. reactive power is added into the equations)
            constraint_power_balance_flx_ne_PQshed_PQstrg(pm, i, nw=n)
        end
...
#new storage_ne constraints
         for i in ids(pm, :ne_storage, nw=n)
             constraint_PQstrg_new_chr_dischr_exclusivity(pm, i, nw=n)
             ...
             constraint_storage_new_sc_ps_and_sd_pd_qd(pm, i, nw=n)
