...
function run_opf_mn_flx_PQshed_Pstrg(file, model_type::Type, optimizer; kwargs...)
    return run_model(file, model_type, optimizer, build_mn_opf_flx_PQshed_Pstrg; ref_extensions= [ref_add_on_off_va_bounds!,ref_add_ne_branch!,ref_add_ne_storage!], kwargs...)
end



function build_mn_opf_flx_PQshed_Pstrg(pm::AbstractPowerModel)
...
# Power Flow constraints
        ...
        for i in ids(pm, :bus, nw=n)            # Nodal balance equation (the equations is modifed with respect to tnep_gen_sh.jl , in addition to the flexible load,   1.new storage modify the power balance equation 2. reactive power is added into the equations)
            constraint_power_balance_flx_ne_PQshed_Pstrg(pm, i, nw=n)
        end
