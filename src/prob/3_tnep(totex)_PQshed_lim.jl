...
export run_tnep_PQshed_lim



function run_tnep_PQshed_lim(file, model_type::Type, optimizer; kwargs...)
    return run_model(file, model_type, optimizer, build_tnep_PQshed_lim; ref_extensions=[ref_add_on_off_va_bounds!,ref_add_ne_branch!], kwargs...)
end

"the general form of the tnep optimization model"
# THE NAME changed from build_tnep_Pshed to build_tnep_PQshed
function build_tnep_PQshed_lim(pm::AbstractPowerModel)
...
# are added for active flexibility
    variable_pflex(pm)
    variable_pflex_minus(pm)
    variable_pflex_plus(pm)
# added for reactive flexibility (NEW)
    variable_qflex(pm)
    variable_qflex_minus(pm)
    variable_qflex_plus(pm)
...
###########################################################
    ...
    for i in ids(pm,:load)
        ...
        constraint_flexible_reactive_Power_flow(pm,i)

        constraint_limited_active_Power_flow(pm,i)                              # ADDED
        constraint_limited_reactive_Power_flow(pm,i)                            # ADDED
    end


    for i in ids(pm, :bus)
        #constraint_power_balance_ne_ls (pm, i)      REMOVED
        constraint_PQ_power_balance_ne_ls(pm, i)   # ADDED
    end
#########################################
    #print(pshh)
    #for i in ids(pm, :load)
