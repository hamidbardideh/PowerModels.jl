...
export run_tnep_PQshed



function run_tnep_PQshed(file, model_type::Type, optimizer; kwargs...)
    return run_model(file, model_type, optimizer, build_tnep_PQshed; ref_extensions=[ref_add_on_off_va_bounds!,ref_add_ne_branch!], kwargs...)
end

"the general form of the tnep optimization model"
# THE NAME changed from build_tnep_Pshed to build_tnep_PQshed
function build_tnep_PQshed(pm::AbstractPowerModel)
...
###########################################################
    ...
    for i in ids(pm,:load)
        ...
        constraint_flexible_reactive_Power_flow(pm,i)   # ADDED
    end
